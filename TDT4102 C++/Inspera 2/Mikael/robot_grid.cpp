...
void RobotGrid::draw_grid_lines()
{
    ...
    for (int i = 0; i < cols+1; i++) { window.draw_line({x_pos+(i*cell_width), y_pos}, {x_pos+(i*cell_width), y_pos+(cell_height*rows)}, Color::black); }
    for (int j = 0; j < rows+1; j++) { window.draw_line({x_pos, y_pos+(j*cell_height)}, {x_pos+(cell_width*cols), y_pos+(j*cell_height)}, Color::black); }
...
Point RobotGrid::get_grid_cell_center_coord(int x, int y) const
{
  ...
  return {x_pos + cell_width/2 + x*cell_width, y_pos + cell_height/2 + y*cell_height};
...
void RobotGrid::make_robot(string name, Point pos, Color color)
{
  ...
  robots[name] = make_unique<Robot>(name, pos, color); //allerde sjekket om eksisterer, om ikke for det ville jeg skrevet insert
...
void RobotGrid::draw_robots()
{
  ...
  for (const auto& robot : robots) {
    window.draw_circle(get_grid_cell_center_coord(robot.second->pos), min(cell_height/2.0, cell_width/2.0), robot.second->color);
...
void RobotGrid::delete_robot(string name)
{
  ...
  robots.erase(name);         // Om draw_robots ikke blir kalt hele tiden av programmet, vil ikke denne alene funke. Dermed den komenterte ut koden.
  // draw_robots();
...
void RobotGrid::move_robot(string name, Point pos)
{
  ...
  robots.at(name)->pos = pos;     // Om draw_robots ikke blir kalt hele tiden av programmet, vil ikke denne alene funke. Dermed den komenterte ut koden.
  // draw_robots();
...
void RobotGrid::recolor_robot(string name, Color color)
{
  ...
  robots.at(name)->color = color;   // Om draw_robots ikke blir kalt hele tiden av programmet, vil ikke denne alene funke. Dermed den komenterte ut koden.
  // draw_robots();
...
void RobotGrid::rename_robot(string name, string new_name)
{
  ...
  robots[new_name] = std::move(robots.at(name));    // Flytter Robot pointer fra det gamle navnet til det nye samtidig som det nye navet blir lagt til i mappet.
  //gjorde feil skal være her: delete_robot(name);   
  robots.at(new_name)->name = new_name;             // Her ender vi det faktiske navnet til roboten
  delete_robot(name);                               // Og her sletter vi til slutt det gamle navnt fra minnet (så kan det ikke kalles på igjenn).
...
void RobotGrid::check_coord_bounds(Point p) const
{
  ...
  if ((p.x > cols || p.x < 0) || (p.y > rows || p.y < 0)) { throw string("The values for this point are invalid! The point is out of bounds."); }
...
void RobotGrid::check_name_available(string name) const
{
  ...
  if (robots.find(name) != robots.end()) {
    throw invalid_argument("Name: '" + name + "', is allready in use!");
  }
